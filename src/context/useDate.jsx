...
const BASE_URL = 'https://we-care-server-seven.vercel.app/api/v1'
function DateContext({ children }) {
    ...
    async function handleDelete(id) {
        try {
            ...
            const response = await fetch(`${BASE_URL}/nurses/${id}`, {
                method: 'DELETE',
                headers: {
                    'Authorization': `Bearer ${token}`
                }
    ...
    return <ProviderData.Provider value={{
        loadingBooking, bookings, token, setToken,
        data, loading, users, getNurseLoading, nurses, city, nurseLoading, handleDelete
    }}>
        {children}
...
function useData() {
    ...
    if (context === undefined) throw new Error('Using context values Outside the context area');
    return context;
}
